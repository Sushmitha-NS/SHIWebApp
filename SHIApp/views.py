...
from .serializers import PropertyListSerializer,PropertyCarouselSerializer  # This is used to get the PropertyListSerializer class to convert it into python obj to json obj.
from .models import (PropertyList, PropertyImages, UserContactData, PropertyImages, Amenities, Builder, UserResumeData, PropertyContact) # these are the models that we will be using for filteration, storing the data into the database.
...
def home(request):
    ...
    """
    PropImages = PropertyList.objects.values('Property_Image', 'slug', 'Location')   #Filter all the Property List objects and insert into PropImages variable.
    Ind_PropDataDict = {
        'carousel_image' : PropImages, #property Images.
    }  # Creating the Prop_Data dictionary to send it in the front end template.
    return render(request, 'index.html', Ind_PropDataDict)
    ...
    try:
        ...
        if SearchQuery != 0:
            ...
            if "+" in SearchQuery:
                ...
                SearchName = SearchQuery.split("+") # replacing the SearchQuery + sign with space to get the exact query.
                FinalResult=0 #here we are storing the PropertyFiltered in the FinalResult variable that will go with the Response.
                for queries in SearchName:
                    PropertyFiltered = PropertyList.objects.filter(SearchName__icontains=queries)#Here we are filtering the data with (PropertyName, Property_Type, Location,SubPropertyType, BuilderName) from (models.py) by using the PropertyList model.
                    if PropertyFiltered != None:
                        FinalResult = PropertyFiltered 
...
def HotPropertyView(request):
    ...
    try:
        ...
        HotPropertiesData = PropertyList.objects.filter(HotProperty__icontains='Yes') #get all the Hotproperties from HotPropertiesList model.
        serializer = PropertyListSerializer(HotPropertiesData, many=True) #by using HotPropertyListSerializer it will convert the data into json format, many=True means all the Fields would send from the model.
        return Response(serializer.data)
    except Exception:
        HotPropertiesData = 0
        return Response(HotPropertiesData)
...
@api_view(['GET'])  # decorator with GET request permission we will use this when working with REST API.
def UpcomingProperty(request):
     #This function will access the GET request from UpcomingProp.js and in response it will return the json of all the related data.
    try:
        #This block will try to execute the data and then return the response.
        UpcomingPropertiesData = PropertyList.objects.filter(UpcomingProperty__icontains='Yes') #get all the Hotproperties from HotPropertiesList model.
        serializer = PropertyListSerializer(UpcomingPropertiesData, many=True) #by using HotPropertyListSerializer it will convert the data into json format, many=True means all the Fields would send from the model.
        return Response(serializer.data)
    except Exception:
        UpcomingPropertiesData = 0
        return Response(UpcomingPropertiesData)

@api_view(['GET'])  # decorator with GET request permission we will use this when working with REST API.
def HomePageSlider(request):
     #This function will access the GET request from Carousel.js and in response it will return the json of all the related data.
    try:
        #This block will try to execute the data and then return the response.
        SliderData = PropertyList.objects.all() #get all the Hotproperties from HotPropertiesList model.
        serializer = PropertyCarouselSerializer(SliderData, many=True) #by using HotPropertyListSerializer it will convert the data into json format, many=True means all the Fields would send from the model.
        return Response(serializer.data)
    except Exception:
        SliderData = 0
        return Response(SliderData)